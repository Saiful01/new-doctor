    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.doctors.update", [$doctor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('specialists') ? 'is-invalid' : '' }}" name="specialists[]" id="specialists" multiple required>
                    @foreach($specialists as $id => $specialist)
                        <option value="{{ $id }}" {{ (in_array($id, old('specialists', [])) || $doctor->specialists->contains($id)) ? 'selected' : '' }}>{{ $specialist }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('hospitals') ? 'is-invalid' : '' }}" name="hospitals[]" id="hospitals" multiple required>
                    @foreach($hospitals as $id => $hospital)
                        <option value="{{ $id }}" {{ (in_array($id, old('hospitals', [])) || $doctor->hospitals->contains($id)) ? 'selected' : '' }}>{{ $hospital }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('days') ? 'is-invalid' : '' }}" name="days[]" id="days" multiple required>
                    @foreach($days as $id => $day)
                        <option value="{{ $id }}" {{ (in_array($id, old('days', [])) || $doctor->days->contains($id)) ? 'selected' : '' }}>{{ $day }}</option>
                    @endforeach
                </select>
                @if($errors->has('days'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('short_details') ? 'is-invalid' : '' }}" name="short_details" id="short_details">{{ old('short_details', $doctor->short_details) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('overview') ? 'is-invalid' : '' }}" name="overview" id="overview">{!! old('overview', $doctor->overview) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fee') ? 'is-invalid' : '' }}" type="number" name="fee" id="fee" value="{{ old('fee', $doctor->fee) }}" step="0.01" required>
                @if($errors->has('fee'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fee') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('old_fee') ? 'is-invalid' : '' }}" type="number" name="old_fee" id="old_fee" value="{{ old('old_fee', $doctor->old_fee) }}" step="0.01">
